import {
  ...
  import { assignUser, getAlternateToAdd, getUserToAdd } from "@/libs/api/methods/admin";
  import { debounce } from "@/libs/utils/optimization";
  
  
  
  
  export default function ListAlternateToAdd({ modal, form }: IModalProps) {
    ...
    const { status, data, error } = useQuery({
      queryKey: ["alternateAddDep",searchTerm],
      queryFn: () => getAlternateToAdd({ department_id2: form?.getFieldValue("id"),  ...(searchTerm && { name: searchTerm }) }),
      enabled: isOpen
    });
