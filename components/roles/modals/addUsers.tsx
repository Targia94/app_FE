...
import { addRoleToUserAPI, getUsersExceptRolesAPI, getUsersRolesAPI } from "@/libs/api/methods/admin";




export default function ListUsers({ modal }: IModalProps) {
  ...
  const { status, data, error } = useQuery({
    queryKey: ["usersToRole", roleId, searchTerm],
    queryFn: () => getUsersExceptRolesAPI({ roleId, ...(searchTerm && { name: searchTerm }) }),
    enabled: isOpen
  });
  ...
  const handleSave = async () => {
    ...
    try {
      for (const userId of userIds) {
        await addRoleToUserAPI({ userId, roleId });
      }
